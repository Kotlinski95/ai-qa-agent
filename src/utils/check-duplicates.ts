...
async function checkDuplicates() {
  ...
  console.log(`📊 Total records in namespace: ${stats.namespaces?.[PINECONE_NAMESPACE]?.recordCount || 0}\n`);

  // To find duplicates, we need to query and check metadata
  // This is a simplified check - in production you'd want to fetch all vectors
...
async function clearAllDuplicates() {
  ...
  try {
    ...
    console.log('💡 Next request will fetch fresh data from website and cache without duplicates\n');
  } catch (error) {
...
if (command === 'clear') {
  ...
  console.log('  node dist/utils/check-duplicates.js clear  - Clear all vectors (removes duplicates)\n');
